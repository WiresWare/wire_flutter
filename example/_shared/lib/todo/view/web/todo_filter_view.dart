...
class TodoFilterView extends DomElement {
  TodoFilterView(HTMLUListElement dom) : super(dom) {
    Wire.data<FilterValues>(DataKeys.FILTER).subscribe((filter) async {
      ...
      (dom.children.item(selectedChildIndex)?.children.item(0) as HTMLElement?)?.className = CLASS_NAME_SELECTED;
    });
  }

  static const Map<FilterValues, int> _FILTER_TO_VALUE = {FilterValues.ALL: 0, FilterValues.ACTIVE: 1, FilterValues.COMPLETED: 2};
